...
| Cluster shard error | Use 3–128 shards and 1–5 total members per shard. |
...
| ACL user rejection | Verify the same-namespace password Secret, `password` key, password length, username, and ACL syntax. |
...
mismatch, untolerated taints, PVC topology conflicts, and namespace quotas.
Required `AntiAffinity` can need one eligible node per data Pod.
