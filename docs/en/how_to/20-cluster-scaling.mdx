...
- For scale-up, ensure capacity for every new shard member and its PVC.
...
If ACL authentication is required, add `--user <username> --askpass` and allocate
a terminal with `kubectl exec -it`.
