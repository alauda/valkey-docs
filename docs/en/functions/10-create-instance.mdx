...
- Set CPU and memory requests and limits. Equal request and limit values avoid a
  validating-webhook warning and provide predictable memory behavior.
...
| `spec.storage` | Omit for ephemeral storage, or select capacity, StorageClass, and access mode. |
...
Then inspect the observed nodes, child resources, Pods, Services, and PVCs:
