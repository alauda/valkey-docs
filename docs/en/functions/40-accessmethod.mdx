...
Use a client identity issued by the same trusted CA. The following form avoids
putting an access control list (ACL) password on the command line, but it checks
only the Cluster bootstrap connection:
...
See the official [Valkey TLS documentation](https://valkey.io/topics/encryption/)
for server and client certificate behavior.
